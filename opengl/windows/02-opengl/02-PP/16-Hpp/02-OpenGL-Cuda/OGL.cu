...
// cuda related headers and libraries
...
void display(void)
{
    ...
    if (onGPU == TRUE) {
        ...
        if (cudaResult != cudaSuccess) {
            fprintf(gpFile, "display: cudaGraphicsMapResources Failed\n");
            uninitialize();
            exit(-1);
        }
        
        // get host compatible mapped pointer for graphics resource
        float4* pPosition = NULL;
        cudaResult = cudaGraphicsResourceGetMappedPointer((void**)&pPosition, NULL, cuGraphicsResource);
        if (cudaResult != cudaSuccess) {
            fprintf(gpFile, "display: cudaGraphicsResourceGetMappedPointer Failed\n");
            uninitialize();
            exit(-1);
        }
        ...
        // pPosition is nothing but vbo_gpu hence bind with it
        cudaResult = cudaGraphicsUnmapResources(1, &cuGraphicsResource, 0);
        if (cudaResult != cudaSuccess) {
            fprintf(gpFile, "display: cudaGraphicsUnmapResources Failed\n");
            uninitialize();
            exit(-1);
        }
...
int initialize(void)
{
    ...
    if (cudaResult != cudaSuccess) {
        fprintf(gpFile, "cudaGetDevice Failed\n");
        uninitialize();
        exit(-1);
    } else if (devCount == 0) {
    ...
    cudaResult = cudaGraphicsGLRegisterBuffer(&cudaGraphicsResource, vbo_gpu, cudaGraphicsMapFlagsWriteDiscard);
    if (cudaResult != cudaSuccess) {
