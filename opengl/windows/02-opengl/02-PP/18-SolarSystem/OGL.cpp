...
void display(void)
{
    ...
    mat4 viewMatrix = vmath::lookat(vec3(0.0f, 0.0f, 5.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f));

    // Sun
    mat4 modelMatrix = vmath::rotate(90.0f, 1.0f, 0.0f, 0.0f);
    modelMatrix *= vmath::scale(0.75f, 0.75f, 0.75f);
    mat4 mvpMatrix = perspectiveProjectionMatrix * viewMatrix * modelMatrix;

    glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, mvpMatrix);
    glUniform4f(colorUniform, 1.0f, 1.0f, 0.0f, 1.0f); // Yellow Sun
    ...
    // Earth
    modelMatrix = vmath::rotate(year, 0.0f, 1.0f, 0.0f); // Revolve
    modelMatrix *= vmath::translate(1.5f, 0.0f, 0.0f);
    mat4 earthBaseMatrix = modelMatrix; // Store this state for Moon

    modelMatrix *= vmath::rotate(90.0f, 1.0f, 0.0f, 0.0f); // Fix poles
    modelMatrix *= vmath::rotate(date, 0.0f, 0.0f, 1.0f); // Spin
    modelMatrix *= vmath::scale(0.2f, 0.2f, 0.2f);
    mvpMatrix = perspectiveProjectionMatrix * viewMatrix * modelMatrix;

    glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, mvpMatrix);
    glUniform4f(colorUniform, 0.4f, 0.7f, 1.0f, 1.0f); // Blue Earth
    
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    glDrawElements(GL_TRIANGLES, gNumElements, GL_UNSIGNED_SHORT, 0);

    // Moon
    modelMatrix = earthBaseMatrix;
    modelMatrix *= vmath::translate(0.5f, 0.0f, 0.0f); // Offset from Earth
    modelMatrix *= vmath::rotate(90.0f, 1.0f, 0.0f, 0.0f);
    modelMatrix *= vmath::rotate(date, 0.0f, 0.0f, 1.0f);
    modelMatrix *= vmath::scale(0.075f, 0.075f, 0.075f);
    mvpMatrix = perspectiveProjectionMatrix * viewMatrix * modelMatrix;

    glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, mvpMatrix);
    glUniform4f(colorUniform, 0.5f, 0.5f, 0.5f, 1.0f); // Grey Moon
