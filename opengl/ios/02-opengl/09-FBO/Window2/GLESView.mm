...
-(int) initialize {
    ...
    glBindAttribLocation(shaderProgramObject, AMC_ATTRIBUTE_TEXTCORD, "aTexCoord");        glLinkProgram(shaderProgramObject);
    ...
    // create fbo and if successful call init_sphere
        if ([self createAndPrepareFBOForDrawing:FBO_WIDTH :FBO_HEIGHT] == TRUE) {
            printf( "fbo creation successful\n");
            ...
            if (fboResult != 0) {
                ...
                printf( "initialize_sphere success\n");
            }
        } else {
...
-(BOOL) createAndPrepareFBOForDrawing:(GLint)texture_width :(GLint) texture_height {
    ...
    if (maxRenderBufferSize < texture_width || maxRenderBufferSize < texture_height) {
        printf("fbo widht/height exceeding max size\n");
        return FALSE;
    }
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        printf( "fbo completion incomplete\n");
        return FALSE;
    }
