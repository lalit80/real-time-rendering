...
public class GLESView extends GLSurfaceView implements OnGestureListener, OnDoubleTapListener, GLSurfaceView.Renderer {
    ...
    private float lightAngle[] = new float[3];
    ...
    private int inititalize(GL10 gl) {
        ...
        light[2].diffuse[1] = 1.0f;
        light[2].diffuse[2] = 0.0f;
        light[2].diffuse[3] = 1.0f;

        light[2].specular[0] = 0.0f;
        light[2].specular[1] = 1.0f;
        light[2].specular[2] = 0.0f;
    ...
    private void update() {
        ...
        x = (float)(Math.cos(lightAngle[0]) * radius);
        y = (float)(Math.sin(lightAngle[0]) * radius);
        light[0].position[0] = x;
        light[0].position[1] = y;
        
        // update position of light 1 (x-z plane)
        x = (float)(Math.cos(lightAngle[1]) * radius);
        z = (float)(Math.sin(lightAngle[1]) * radius);
        light[1].position[0] = x;
        light[1].position[2] = z;
        
        // update position of light 2 (z-y plane)
        z = (float)(Math.cos(lightAngle[2]) * radius);
        y = (float)(Math.sin(lightAngle[2]) * radius);
        light[2].position[1] = y;
        light[2].position[2] = z;
    }
