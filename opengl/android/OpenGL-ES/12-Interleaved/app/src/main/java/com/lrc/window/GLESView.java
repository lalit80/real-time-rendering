...
public class GLESView extends GLSurfaceView implements OnGestureListener, OnDoubleTapListener, GLSurfaceView.Renderer {
    ...
    private int LaUniform = 0;                   // ambient light
    private int LdUniform = 0;                   // diffuse
    private int LsUniform = 0;                   // specular
    private int KaUniform = 0;                   // ambient material
    ...
    private int inititalize(GL10 gl) {
        ...
        FloatBuffer cube_PCNTBuffer = byteBuffer.asFloatBuffer();
        cube_PCNTBuffer.put(cube_PCNT);
        cube_PCNTBuffer.position(0);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (24 * 11 * 4), cube_PCNTBuffer, GLES32.GL_STATIC_DRAW);
        ...
        GLES32.glVertexAttribPointer(MyAttributes.AMC_ATTRIBUTE_TEXTCORD, 2, GLES32.GL_FLOAT, false, 11 * 4, 9 * 4);
        ...
        material[0].ambient[0] = 0.25f;
        material[0].ambient[1] = 0.25f;
        material[0].ambient[2] = 0.25f;
        ...
        texMarble[0] = loadGLTexture(R.raw.marble);
    ...
    private void display() {
        ...
        Matrix.setRotateM(rotationMatrixZ, 0, angleRectangle, 0.0f, 0.0f, 1.0f); 
        Matrix.setRotateM(rotationMatrixY, 0, angleRectangle, 0.0f, 1.0f, 0.0f); 
        Matrix.setRotateM(rotationMatrixX, 0, angleRectangle, 1.0f, 0.0f, 0.0f); 

        Matrix.multiplyMM(tempRotationMatrix, 0, rotationMatrixY, 0, rotationMatrixZ, 0);
        Matrix.multiplyMM(rotationMatrix, 0, rotationMatrixX, 0, tempRotationMatrix, 0);
