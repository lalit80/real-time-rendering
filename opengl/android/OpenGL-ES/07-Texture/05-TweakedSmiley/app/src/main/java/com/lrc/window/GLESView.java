...
public class GLESView extends GLSurfaceView implements OnGestureListener, OnDoubleTapListener, GLSurfaceView.Renderer {
    ...
    private int inititalize(GL10 gl) {
        ...
        final float triangle_texcoord[] = new float[] {   
                                                // front
        0.5f, 1.0f, // front-top
        ...
        // color
        GLES32.glGenBuffers(1, triangle_vbo_texcoord, 0);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, triangle_vbo_texcoord[0]);
        byteBuffer = ByteBuffer.allocateDirect(triangle_texcoord.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        FloatBuffer triangleColorBuffer = byteBuffer.asFloatBuffer();
        triangleColorBuffer.put(triangle_texcoord);
        triangleColorBuffer.position(0);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (triangle_texcoord.length * 4), triangleColorBuffer, GLES32.GL_STATIC_DRAW);
        ...
        texSmiley[0] = loadGLTexture(R.raw.smiley);
        texKundli[0] = loadGLTexture(R.raw.vk);
    ...
    private void display() {
        ...
        float cube_texcoord[] = new float[8];
        ...
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, rectangle_vbo_texcoord[0]);
        ByteBuffer byteBuffer = ByteBuffer.allocateDirect(cube_texcoord.length * 4);
        byteBuffer.order(ByteOrder.nativeOrder());
        FloatBuffer rectangleColorBuffer = byteBuffer.asFloatBuffer();
        rectangleColorBuffer.put(cube_texcoord);
        rectangleColorBuffer.position(0);
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, (4 * 2 * 4), rectangleColorBuffer, GLES32.GL_DYNAMIC_DRAW);
        GLES32.glVertexAttribPointer(MyAttributes.AMC_ATTRIBUTE_TEXTCORD, 2, GLES32.GL_FLOAT, false, 0, 0);
        GLES32.glEnableVertexAttribArray(MyAttributes.AMC_ATTRIBUTE_TEXTCORD);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
    ...
    private void uninitialize() {
        if (triangle_vbo_texcoord[0] > 0) {
            GLES32.glDeleteBuffers(1, triangle_vbo_texcoord, 0);
            triangle_vbo_texcoord[0] = 0;
        }
