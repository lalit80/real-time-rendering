...
HRESULT initialize(void)
{
    ...
    const char* vertexShaderSourceCode =
        ...
        "};" \
        "vertex_output main(float4 pos : POSITION, float3 color: COLOR, float3 norm : NORMAL, float2 tex: TEXCOORD) {" \
        ...
        "       output.lightDirection = (float3)lightPosition.xyz - eyeCoordinates.xyz;" \
    ...
    const char* pixelShaderSourceCode =
        ...
        "float4 main(vertex_output input) : SV_TARGET {" \
            ...
            "   float3 ambient = la.rgb * ka.rgb;" \
            "   float tn_dot_ld = max(dot(normalized_transformed_normals, normalized_light_direction), 0.0);" \
            "   float3 diffuse = ld.rgb * kd.rgb * tn_dot_ld;" \
            "   float3 reflection_vector = reflect(-normalized_light_direction, normalized_transformed_normals);" \
            "   float3 specular = ls.rgb * ks.rgb * pow(max(dot(reflection_vector, normalized_viewer_vector), 0.0), materialShininess);" \
            "   phong_ads_color = ambient + diffuse + specular;" \
            "}" \
            "else {" \
            "   phong_ads_color = float3(1.0, 1.0, 1.0);" \
            "}" \
            "float4 texColor = myTexture.Sample(mySampler, input.texcoord);" \
            "return texColor * float4(phong_ads_color, 1.0) * float4(input.color, 1.0);" \
        "}";
    ...
    d3d11InputElementDesc[1].InputSlot = 0;
    d3d11InputElementDesc[1].AlignedByteOffset = 12;
    ...
    d3d11InputElementDesc[2].InputSlot = 0;
    d3d11InputElementDesc[2].AlignedByteOffset = 24;
    ...
    d3d11InputElementDesc[3].InputSlot = 0;
    d3d11InputElementDesc[3].AlignedByteOffset = 36;
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubResourceData, &gpID3D11Buffer_VertexBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for vertex buffer failed\n");
        fclose(gpFile);
        return hr;
    } else {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for vertex buffer successful\n");
        fclose(gpFile);
    }
    ...
    // load texture
    hr = loadD3DTexture(L"marble.bmp", &gpID3D11ShaderResourceView_CubeTexture);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "loadD3DTexture() failed for marble.bmp\n");
        fclose(gpFile);
        return hr;
    }
...
void display(void)
{
    ...
    XMMATRIX rotationMatrix = XMMatrixRotationX(angleCube) *
                              XMMatrixRotationY(angleCube) *
                              XMMatrixRotationZ(angleCube);
