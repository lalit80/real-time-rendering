...
ID3D11Buffer* gpID3D11Buffer_Pyramid_PositionBuffer = NULL;
ID3D11Buffer* gpID3D11Buffer_Cube_PositionBuffer = NULL;
ID3D11Buffer* gpID3D11Buffer_Pyramid_TexCoordBuffer = NULL;
ID3D11Buffer* gpID3D11Buffer_Cube_TexCoordBuffer = NULL;
...
HRESULT initialize(void)
{
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Pyramid_PositionBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Pyramid_TexCoordBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for texture buffer failed\n");
        fclose(gpFile);
        return hr;
    } else {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for texture buffer successful\n");
        fclose(gpFile);
    }
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Cube_PositionBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for position buffer failed\n");
        fclose(gpFile);
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Cube_TexCoordBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for rectangle texture buffer failed\n");
        fclose(gpFile);
        return hr;
    } else {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for rectangle texture buffer successful\n");
        fclose(gpFile);
    }
    ...
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "loadD3DTexture() failed for stone.bmp\n");
        fclose(gpFile);
        return hr;
    }

    hr = loadD3DTexture(L"vk.bmp", &gpID3D11ShaderResourceView_CubeTexture);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "loadD3DTexture() failed for vk.bmp\n");
        fclose(gpFile);
        return hr;
    }
...
void display(void)
{
    ...
    UINT offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_Pyramid_PositionBuffer, &stride, &offset);
    gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    // bind vertex buffer for texture
    stride = sizeof(float) * 2;
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Pyramid_TexCoordBuffer, &stride, &offset);
    ...
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_Cube_PositionBuffer, &stride, &offset);
    gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    // bind vertex buffer for texture
    stride = sizeof(float) * 2;
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Cube_TexCoordBuffer, &stride, &offset);
...
void uninitialize(void)
{
    ...
    if (gpID3D11Buffer_Pyramid_PositionBuffer) {
        gpID3D11Buffer_Pyramid_PositionBuffer->Release();
        gpID3D11Buffer_Pyramid_PositionBuffer = NULL;
    }
...
HRESULT loadD3DTexture(const wchar_t* textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView) {
    ...
    if (FAILED(hr)) {
        fprintf(gpFile, "CreateWICTextureFromFile failed for %ls\n", textureFileName);
        return hr;
    }
    return hr;
}
