...
unsigned char cb[CHECKERBOARD_HEIGHT][CHECKERBOARD_WIDTH][4];
...
HRESULT loadD3DTexture(ID3D11ShaderResourceView** ppID3D11ShaderResourceView) {
    ...
    D3D11_TEXTURE2D_DESC d3d11Texture2DDesc;
    ZeroMemory(&d3d11Texture2DDesc, sizeof(D3D11_TEXTURE2D_DESC));
    d3d11Texture2DDesc.Width = CHECKERBOARD_WIDTH;
    d3d11Texture2DDesc.Height = CHECKERBOARD_HEIGHT;
    d3d11Texture2DDesc.ArraySize = 1;
    d3d11Texture2DDesc.MipLevels = 1;
    d3d11Texture2DDesc.SampleDesc.Count = 1;
    d3d11Texture2DDesc.SampleDesc.Quality = 0;
    ...
    D3D11_SUBRESOURCE_DATA d3d11SubresourceData;
    ZeroMemory(&d3d11SubresourceData, sizeof(D3D11_SUBRESOURCE_DATA));
    d3d11SubresourceData.pSysMem = cb;
...
void makeCheckerboard(void)
{
    ...
    for (i = 0; i < CHECKERBOARD_HEIGHT; ++i) {
        for (j = 0; j < CHECKERBOARD_WIDTH; ++j) {
            ...
            cb[i][j][0] = (unsigned char)c;
            cb[i][j][1] = (unsigned char)c;
            cb[i][j][2] = (unsigned char)c;
            cb[i][j][3] = (unsigned char)255;
        }
    }
}
