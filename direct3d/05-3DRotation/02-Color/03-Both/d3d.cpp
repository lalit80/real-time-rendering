...
HRESULT initialize(void)
{
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Pyramid_ColorBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Cube_ColorBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
...
void display(void)
{
    ...
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Pyramid_ColorBuffer, &stride, &offset);
    ...
    stride = sizeof(float) * 3;
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Cube_ColorBuffer, &stride, &offset);
