...
float lightPosition0[] = {-2.0f, 0.0f, 0.0f, 1.0f};
...
HRESULT initialize(void)
{
    ...
    const char* vertexShaderSourceCode =
        ...
        "   float3 out_lightDirection[2] : TEXCOORD1; \n" \
        "   float3 out_viewerVector : TEXCOORD3; \n" \
        "};" \
    ...
    const char* pixelShaderSourceCode =
        ...
        "       float3 normalizedLightDirection[2];" \
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_VertexBuffer_Position);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_VertexBuffer_Normal);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for normal buffer failed\n");
        fclose(gpFile);
        return hr;
    } else {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for normal buffer successful\n");
        fclose(gpFile);
    }
