...
ID3D11Buffer* gpID3D11Buffer_Pyramid_NormalBuffer = NULL;
ID3D11Buffer* gpID3D11Buffer_Cube_NormalBuffer = NULL;
...
float lightPosition[] = {0, 0, -2.0f, 1.0f};
...
HRESULT initialize(void)
{
    ...
    const char* vertexShaderSourceCode =
        ...
        "       output.diffuseLight = ld * kd * max(dot(lightSource, transformedNormal), 0.0f);" \
        "   } else {" \
   ...
   D3D11_INPUT_ELEMENT_DESC d3d11InputElementDesc[2];
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Pyramid_NormalBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
    ...
    hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, &d3d11SubresourceData, &gpID3D11Buffer_Cube_NormalBuffer);
    if(FAILED(hr)) {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for rectangle normal buffer failed\n");
        fclose(gpFile);
        return hr;
    } else {
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "ID3D11Device::CreateBuffer for rectangle normal buffer successful\n");
        fclose(gpFile);
    }
...
void display(void)
{
    ...
    offset = 0;
    gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_Pyramid_NormalBuffer, &stride, &offset);
