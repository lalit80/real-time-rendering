...
HRESULT initialize(void)
{
    ...
    hr = D3DCompile(geometryShaderSourceCode, lstrlenA(geometryShaderSourceCode) + 1,
                    "GS", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "gs_5_0",
                    0, 0, &pID3DBlob_GeometryShaderCode, &pID3DBlob_Error);
    if(FAILED(hr)) {
        if(pID3DBlob_Error != NULL) {
            gpFile = fopen(gszLogFileName, "a+");
            fprintf(gpFile, "D3DCompile for geometry shader failed: %s\n", (char*)pID3DBlob_Error->GetBufferPointer());
            fclose(gpFile);
            pID3DBlob_Error->Release();
            pID3DBlob_Error = NULL;
        }
        ...
        gpFile = fopen(gszLogFileName, "a+");
        fprintf(gpFile, "D3DCompile for geometry shader successful\n");
        fclose(gpFile);
    }
